...
import argparse
from text_new_mdm import init_sentence
...
def generate(instruction, top_p=0.7, num=1):
    ...
    if num==1:
        output_text, num_tokens = get_completion(input_prompt, top_p)
    else:
        input_prompts = instruction
        output_text, num_tokens = get_completion(input_prompts, top_p, num)
    ...
    def get_fitness_score(self, sentences):
        ...
        for i,sentence in enumerate(sentences):
            ...
            if sentence_distance(x_prime_sent, self.target_sent)>0.4:
                ...
                mdm_gen_motion_from_text(self.model, self.cfg, x_prime_sent, x_motion_path, self.device)# , self.length
    ...
    def run(self, log=None):
        ...
        while itr <= self.max_iters:
            ...
            if elite_pop_scores[-1] > best_score:
                ...
                mdm_gen_motion_from_text(self.model, self.cfg, x_prime_sent, x_motion_path_npy, self.device)# , self.length
...
def run_attacks(i, id_list, v_str):
    ...
    # 创建文件夹
    for folder in ['intermediate_motion_path', 'best_motion_path']:
        folder_name = f"attack/mdm_{folder}_{v_str}/{id_str}"
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)
    directory_path = f"attack/mdm_run_log_{v_str}"
    ...
    tar_motion_path = f"attack/mdm_target/{id_str}.npy"
    mdm_gen_motion_from_text(model, cfg, tar_sent, tar_motion_path, device)
    ...
    top_k = 8
    text_length = 20
    log_save_path = f'attack/mdm_run_log_{v_str}/{id_str}.txt'
    intem_motion_path = f'attack/mdm_intermediate_motion_path_{v_str}/{id_str}/'
    best_motion_path = f'attack/mdm_best_motion_path_{v_str}/{id_str}'
    mutate_by_impor = False
    ...
    import json
    with open('attack/ori_sentences_mdm.json', 'r') as f:
