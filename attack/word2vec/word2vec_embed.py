...
class Word2VecSubstitute():
    ...
    def __init__(self, tar_tokens=None, cosine=False, k = 100, threshold = 10, device = None):
        ...
        #load wordvec
        wordvec = LOAD("/Word2Vec/")# 当前工作目录

        self.tar_tokens = tar_tokens
        self.word2id = wordvec.word2id # 每个词对应的编号，共300000个
        self.embedding = torch.from_numpy(wordvec.embedding) # shape=(300000,300)
        self.cosine = cosine
        self.k = k
        ...
        self.pos_tagger = LOAD_perceptron_tagger("/NLTKPerceptronPosTagger/")# 当前工作目录
